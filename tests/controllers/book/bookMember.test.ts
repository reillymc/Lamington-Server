...
test("should save multiple book members for one book", async () => {
...
test("should save one book member for multiple books", async () => {
    const [user] = await CreateUsers();
    const [books] = await CreateBooks({ count: 5, createdBy: user!.userId });

    await BookMemberActions.save(books.map(({ bookId }) => ({ bookId: bookId, members: [{ userId: user!.userId }] })));
...
test("should save multiple book members for multiple books", async () => {
