...
interface TagGroup extends TagBase {
    children: Record<string, TagBase>;
}
...
export const SYSTEM_TAGS = Object.values(DEFINITIONS).flatMap((group) => [
    { tagId: group.tagId, name: group.name, description: group.description },
    ...Object.values(group.children).map((child) => ({
        ...child,
        parentId: group.tagId,
    })),
]);

// Default tags
export const seed = async (knex: Knex): Promise<void> => {
    await knex("tag").insert(SYSTEM_TAGS).onConflict("tagId").merge();
};
...
export const Tags = Object.fromEntries(
    Object.entries(DEFINITIONS).map(([group, def]) => [
        group,
        Object.fromEntries(
            Object.entries(def.children).map(([child, val]) => [
                child,
                val.tagId,
            ]),
        ),
    ...
    [K in keyof Definitions]: {
        [C in keyof Definitions[K]["children"]]: Definitions[K]["children"][C] extends {
            tagId: infer I;
        }
            ? I
            : never;
