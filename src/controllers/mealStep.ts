import db, { lamington, mealStep, MealStep } from "../database";
...
const insertRows = async (mealSteps: MealStep[]) =>
    db(lamington.mealStep).insert(mealSteps).onConflict([mealStep.mealId, mealStep.stepId]).merge();
...
const selectByMealId = async (mealId: string): Promise<MealStepResults> =>
    db(lamington.mealStep)
        .where({ [mealStep.mealId]: mealId })
        .select(mealStep.stepId, mealStep.sectionId, mealStep.index, mealStep.description);
