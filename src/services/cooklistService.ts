...
export const createCooklistService: CreateService<CooklistService, "cooklistRepository"> = (
    database,
    { cooklistRepository }
) => ({
    getMeals: async userId => {
        const { meals } = await cooklistRepository.readAllMeals(database, { userId });
        return meals;
    },
    ...
    updateMeal: (userId, mealId, request) =>
        database.transaction(async trx => {
            const permissions = await cooklistRepository.verifyMealPermissions(trx, { userId, meals: [{ mealId }] });
            const missingPermissions = permissions.meals.some(({ hasPermissions }) => !hasPermissions);

            if (missingPermissions) {
                throw new NotFoundError("cooklist meal", mealId);
            }

            const { meals } = await cooklistRepository.updateMeals(trx, { meals: [{ mealId, ...request }] });
